...
**In laymen's terms**, this software simulates an X-Ray camera orbiting the Earth pointing at our magnetic field. This helps plasma scientists study how our magnetic field is influenced by the sun's solar wind and forms part of a space research mission launching in Summer 2026.
...
# Build Instructions
## Pre-requisites
To run this, you need to have installed <a href="https://github.com/CNES/EleFits">EleFits</a> and <a href="https://heasarc.gsfc.nasa.gov/fitsio/">cfitsio</a>, along with `cmake`. The CMake file in this project (the file which tells the computer how to compile the code) will look for your installed version of EleFits, and EleFits (I think) will need to look for your install of cfitsio. It's a bit finnicky so it's probably best you have a bit of knowledge on building C++ projects. Both EleFits and cfitsio have their own instructions for installing their software in the links above.

## Download and build
1. Once EleFits is installed, download this project code code with `git clone https://github.com/Zach-Clare/birp_cpp.git` into wherever you want to have the files on your computer.
2. Once downloaded, navigate to wherever you downloaded the project in your terminal. In the root of the project, create a build directory with `mkdir build` and navigate into it. The directory can be called whatever you want, it's just convention to call it "build". We'll run the rest of our commands from here.
3. Next we'll prepare the files for compilation. Do this with `cmake -DCMAKE_MODULE_PATH=../cmake/modules/FindEleFits.cmake ..`. We're calling cmake and giving it a flag, telling it where it can find the modules needed. This path, in this project specifically, points to a cmake file that tells cmake where to look for EleFits. It's not an ideal way of doing this but this is down to the EleFits developers as far as I'm aware. If you know a bit about C++ compilation with C++ and know a better way of doing this, please reach out.
4. Now we will actually build the files with `cmake ../` (which preps the preceeding folder and then `cmake --build .` which does the building. Passing the build flag to cmake means we also need to give it a location to put the files which, since we're currently in the build directory, is `.`. These two commands are in `build.sh` if you want to just run that for simplicity.
5. That will create a `birp` executable. You can place the batch files in the `data/batch` folder, pass the input files directly with `-i`, or run CMEM with `-c`. Use `./birp` inside the `build` directory to run it. Your output FITS files will be in `data/output`, or whever you asked them to be with `-o`.
